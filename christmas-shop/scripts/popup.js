...
const svg = `<svg class="snowflake" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
<g clip-path="url(#clip0_13246_840)">
...
function setPopupData(popup, data) {
  ...
  setRating(popup, "live", data.superpowers.live)
  setRating(popup, "create", data.superpowers.create)
  setRating(popup, "love", data.superpowers.love)
  setRating(popup, "dream", data.superpowers.dream)
}

function setRating(popup, type, price) {
  const element = popup.querySelector(`.type-${type}`)
  const elementPrice = popup.querySelector(`.type-${type} .price`)
  elementPrice.textContent = price

  const snowflakesList = element.querySelector(".popup__snowflake-list")
  const activeCount = Math.floor(price / 100)

  snowflakesList.innerHTML = ""
  for (let i = 0; i < 5; i++) {
    const listItem = document.createElement("li")
    listItem.classList.add("popup__snowflake-list-item")
    ...
    snowflakesList.appendChild(listItem)
  }
}
